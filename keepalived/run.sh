...
while true; do
  ...
  while [ -z "$pid" ]; do
    echo "Empezando Keepalived en el fondo..."
    ...
    if [ -z "$pid" ]; then
      echo "El inicio de Keepalived falló, durmiendo 2s, luego volviendo a intentarlo..."
      sleep 2
    fi
  done
...
wait $pid
echo "El proceso Keepalived ya no se está ejecutando, saliendo..."
# Exit with an error
exit 1
